...
max_factor = 2
shrink_factor = max_factor / ( 1 + max_factor )
mode_threshold = 0.5
...
class neuron():
    def __init__(self, input_size, weight_range = 1, *pargs, **kwargs):
        ...
        self.slopes = [ 0 for i in range(input_size) ]
        self.adjusts = [ 0 for i in range(input_size) ]
        self.nablas = None
    ...
    def backprop(self, vector, cost):
        ...
        if self.nablas is None:
            # setting up slopes as decay * weight
            self.nablas = [ -0.001 * v for v in self.weights ]
    ...
    def adjust(self, learning_rate, batch_size = 1):
        for i in range( len(self.weights) ):
            adjust = 0
            if self.adjusts[i] > mode_threshold:
                if self.nablas[i] > 0:
                    adjust += -learning_rate * self.nablas[i]

                if self.nablas[i] > shrink_factor * self.slopes[i]:
                    adjust += max_factor * self.adjusts[i]
                else:
                    adjust += ( self.nablas[i] / (self.slopes[i] - self.nablas[i]) ) * self.adjusts[i]
            elif self.adjusts[i] < -mode_threshold:
                if self.nablas[i] < 0:
                    adjust += -learning_rate * self.nablas[i]
            ...
            self.adjusts[i] = round(adjust, 3)
            self.slopes[i] = round(self.nablas[i], 3)
